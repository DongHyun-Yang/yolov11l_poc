...
ONNX_MODEL_PATH = 'models/yolo11l-seg.onnx'
...
def postprocess_segmentation(outputs, original_width, original_height, ratio, pad):
    """Process segmentation model output to get final masks and bounding boxes."""
    # YOLOv11-seg outputs: [boxes, masks]
    # boxes: [batch, num_detections, 84 + num_masks] (4 box coords + 80 classes + masks coefficients)
    # masks: [batch, num_masks, mask_height, mask_width] (prototype masks)
    
    if len(outputs) < 2:
        return []
    
    predictions = outputs[0]  # Shape: [1, 116, 8400] (4 box + 80 classes + 32 mask coeffs)
    proto_masks = outputs[1]  # Shape: [1, 32, 160, 160] (prototype masks)
    
    predictions = np.squeeze(predictions).T  # Shape: [8400, 116]
    proto_masks = np.squeeze(proto_masks)    # Shape: [32, 160, 160]
    ...
    for row in predictions:
        # Split the prediction: [x, y, w, h, class_scores..., mask_coefficients...]
        box_data = row[:4]  # x, y, w, h
        class_scores = row[4:84]  # 80 class scores
        mask_coeff = row[84:]  # 32 mask coefficients
    ...
    if boxes:
        ...
        if len(indices) > 0:
            for i in indices.flatten():
                ...
                mask = generate_mask(mask_coeffs[i], proto_masks, boxes[i], original_width, original_height, ratio, pad)
...
def generate_mask(mask_coeff, proto_masks, box, original_width, original_height, ratio, pad):
    """Generate final mask from mask coefficients and prototype masks following ultralytics process_mask."""
    # Get prototype mask dimensions
    c, mh, mw = proto_masks.shape  # (32, 160, 160)
    ih, iw = original_height, original_width  # Original image size
    
    # Matrix multiplication: mask_coeff @ proto_masks
    mask = np.dot(mask_coeff, proto_masks.reshape(c, -1))  # (32,) @ (32, 160*160) -> (160*160,)
    mask = mask.reshape(mh, mw)  # Reshape to (160, 160)
    
    # Calculate ratios for downsampled bounding box
    width_ratio = mw / iw   # 160 / original_width
    height_ratio = mh / ih  # 160 / original_height
    ...
    # Crop mask to bounding box (following ultralytics crop_mask)
    mask = crop_mask_np(mask, downsampled_bbox)
    
    # Scale mask to original image size (following ultralytics scale_masks)
    mask = scale_mask_to_original(mask, (ih, iw), (mh, mw))
    
    # Apply threshold to get binary mask (following ultralytics gt_(0.0))
...
def draw_segmentation(image_path, detections, output_path):
...
def process_single_image(image_path, output_dir):
    ...
    try:
        ...
        # 3. Post-processing
        detections = postprocess_segmentation(outputs, original_w, original_h, ratio, pad)
        
        # 4. Generate result save path
        filename = Path(image_path).stem
        output_path = os.path.join(output_dir, f"{filename}_segmented.jpg")
        
        # 5. Visualization
        if detections:
            draw_segmentation(image_path, detections, output_path)
            print(f"[{filename}] Total {len(detections)} objects segmented.")
            for i, det in enumerate(detections):
                class_name = CLASSES[det['class_id']]
                score = det['score']
                box = det['box']
                mask_area = np.sum(det['mask'])
                print(f"  {i+1}. {class_name}: {score:.2f} - Position: ({box[0]}, {box[1]}) ~ ({box[2]}, {box[3]}) - Mask area: {mask_area} pixels")
        else:
...
if __name__ == '__main__':
    ...
    print(f"Processing {len(image_files)} image files for segmentation.")
    ...
    print("-" * 50)
    print(f"Segmentation processing completed: {success_count}/{len(image_files)} successful")
