#!/usr/bin/env python3
"""
Compare segmentation results between ultralytics CLI and our custom implementation
...
from main import letterbox, postprocess_segmentation

PT_MODEL_PATH = 'models/yolo11l-seg.pt'
    ...
    for r in results:
        ...
        if r.masks is not None:
            ...
            for i, mask in enumerate(masks):
                ...
                print(f"  Mask {i+1} area: {mask_area} pixels")
...
def test_our_onnx_implementation():
    ...
    formatted_outputs = [outputs[0], outputs[1]]  # [predictions, proto_masks]
    
    # Use main.py postprocess_segmentation function for complete processing
    detections = postprocess_segmentation(formatted_outputs, original_width, original_height, ratio, pad)
    ...
    for i, det in enumerate(detections):
        ...
        print(f"  {i+1}. {class_name}: {score:.2f} - Position: ({x1}, {y1}) ~ ({x2}, {y2}) - Mask area: {mask_area} pixels")
...
def test_main_with_onnx():
    ...
    # Post-process using main.py seg functions
    start_postprocess = time.time()
...
if __name__ == "__main__":
    print("🚀 Performance Comparison: PT vs ONNX Models")
    print("=" * 80)
